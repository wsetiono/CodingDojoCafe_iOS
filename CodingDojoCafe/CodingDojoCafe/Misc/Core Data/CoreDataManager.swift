...
class CoreDataManager: NSObject {
    ...
    override init() {
        container = NSPersistentContainer(name: "CodingDojoCafeModel")
    ...
    func addPhoto(_ item:RestaurantPhotoItem) {
        let photo = RestaurantPhoto(context:
    ...
    func fetchReviews(by identifier:Int) -> [ReviewItem] {
        ...
        let predicate = NSPredicate(format:
                        "restaurantID = %i", Int32(identifier))
    ...
    func fetchPhotos(by identifier:Int) -> [RestaurantPhotoItem] {
        let moc = container.viewContext
        let request:NSFetchRequest<RestaurantPhoto> =
             RestaurantPhoto.fetchRequest()
        let predicate = NSPredicate(format:
                        "restaurantID = %i", Int32(identifier))
